...
class DailyCollections extends Formsheet{
	...
	function data($data,$total_page,$page){
		...
		$y=0.8;
		$this->GRID['font_size']=9;
		$this->centerText(0,$y,'Date',2,'');
		$this->centerText(2,$y,'Day',2,'');
		$this->leftText(4.2,$y,'Description','','');
		$this->centerText(11,$y,'Collection',2,'');
		$this->centerText(13,$y++,'Balance',2,'');
		
		$totalcollectionperpage = 0;
		foreach($data as $d){
			$totalcollectionperpage+= $d['collection'];
			$this->centerText(0,$y,$d['date'],2,'');
		...
		$this->rightText(11,$y,'Total Collection Per Page:','','');
